...
function ChatsNavigator(props) {
  ...
  useEffect(() => {
    if (
      props.inComingMessage != undefined &&
      (props.inComingMessage?.length !== 0 || props.sellerData !== undefined)
    ) {
        ...
        tempCustomUserList = uniqueUserIDsList?.map((id) => {
          ...
          return {
            ...
            unreadCount: inComingMessage.filter(
              (object) =>
                object.attributes.from.data.id === id &&
                object.attributes.read === false
            )?.length,
          };
        });
      } else {
      ...
      if (
        ...
        tempCustomUserList.findIndex(
          (obj) => obj.id === props.sellerData.id
        ) === -1
      ) {
      ...
      tempCustomUserList?.map((item) => {
        if (!findIdExistById(UniqueUser, item.id)) {
          ...
          props.incomingFromSocket !== undefined &&
          item.id === props.incomingFromSocket.attributes.from.data.id
        ) {
          setUserList((prev) => {
            return prev?.map((object) => {
              if (
                object.id === props.incomingFromSocket.attributes.from.data.id
              ) {
    ...
    const getSellerInfo = async () => {
      const response = await callApi({
        url:
          ...
          `/users?pagination[start]=0&pagination[limit]=10&filters[id][$ne]=${props.userLoginData.id}&filters[username][$contains]=${searchKey}`,
        method: "get",
        params: {
          populate: {
            product: {
    ...
    return (
      <Box>
        ...
        >
          {searchKey?.length > 0 ? (
            <span>
              Kết quả tìm kiếm &#40;Có {searchUserList?.length} kết quả&#41;
            </span>
          ) : (
                  ...
                  onClick={() => {
                    ...
                    setUserList((prev) => {
                      if (prev.findIndex((obj) => obj.id === item.id) === -1) {
                        ...
                        setUniqueUser((subPrev) => [
                          ...subPrev,
                          { id: item.id, unreadCount: 0 },
                        ]);
                        return [...prev, { ...item, unreadCount: 0 }];
                      } else {
                        setSelectedIndex(
                          prev.findIndex((obj) => obj.id === item.id)
                        );
                        return prev;
                      }
                    });
